# Biona — Claude CLI Implementation Prompts
## Biona Speech Intelligence Engine · Full Implementation Playbook

> **How to use this file**
> Each section contains a numbered prompt to paste into Claude CLI (in VS Code terminal).
> Run them **in order within each track**. System A and System B prompts can be run
> in parallel across two VS Code windows / terminals.
> The prompts are self-contained — each one tells Claude CLI exactly what files to create.
...
- `biona/engine/`   → System A: C++17 on-device inference engine
- `biona/training/` → System B: Python training and labeling pipeline
...
- `engine/`   — On-device C++17 inference engine (System A)
- `training/` — Server-side Python training pipeline (System B)

## System Boundary Rule
**System A runs on-device. System B runs on servers. These are separate systems.**
LLM inference, cloud APIs, and large-batch GPU processing belong in System B ONLY.
The ONNX model produced by System B is the only artifact that crosses the boundary.
...
# SYSTEM A — C++ On-Device Engine
## Track A: Run these prompts in order in `biona/engine/`
...
Work inside the directory: biona/engine/
...
biona/engine/core/include/biona/core/
...
The core type headers already exist at biona/engine/core/include/biona/core/

Create the following pure abstract interface headers in
biona/engine/core/include/biona/core/interfaces/
...
- System A (C++ runtime) expects to consume
- System B (PyTorch training) must produce on export

Create: biona/engine/core/include/biona/core/onnx_contract.hpp
...
- System B's torch.onnx.export call must match these names exactly
...
Existing headers: biona/engine/core/include/biona/core/

Create biona/engine/security/include/biona/security/safe_log.hpp
and     biona/engine/security/src/safe_log.cpp
...
Include unit tests in biona/engine/tests/unit/test_safe_log.cpp using
...
Create:
  biona/engine/security/include/biona/security/secret_manager.hpp
  biona/engine/security/src/secret_manager_env.cpp   ← environment var impl
  biona/engine/security/src/secret_manager_factory.cpp
...
Existing: biona/engine/security/include/biona/security/
           safe_log.hpp, secret_manager.hpp

Create:
  biona/engine/security/include/biona/security/model_loader.hpp
  biona/engine/security/src/model_loader.cpp
  biona/engine/security/include/biona/security/secure_buffer.hpp
...
Existing interface: biona/engine/core/include/biona/core/interfaces/vad.hpp

Create:
  biona/engine/core/src/vad/webrtc_vad.hpp
  biona/engine/core/src/vad/webrtc_vad.cpp
  biona/engine/core/src/vad/energy_vad.hpp   ← fallback
  biona/engine/core/src/vad/energy_vad.cpp
  biona/engine/core/src/vad/vad_factory.cpp  ← implements createVAD()
...
Include unit tests in biona/engine/tests/unit/test_vad.cpp:
...
Existing files:
  biona/engine/core/include/biona/core/interfaces/inference_engine.hpp
  biona/engine/core/include/biona/core/onnx_contract.hpp
  biona/engine/security/include/biona/security/model_loader.hpp
  biona/engine/security/include/biona/security/safe_log.hpp

Create:
  biona/engine/engines/onnx/include/biona/engines/onnx_engine.hpp
  biona/engine/engines/onnx/src/onnx_engine.cpp
...
Create:
  biona/engine/core/include/biona/core/spsc_queue.hpp
  biona/engine/core/include/biona/core/signal_thread_pool.hpp
  biona/engine/core/src/signal_thread_pool.cpp
...
Create:
  biona/engine/sdk/include/biona/biona.h        ← Public C header (extern "C")
  biona/engine/sdk/src/biona_sdk.cpp            ← C++ implementation of C API
  biona/engine/sdk/include/biona/biona.hpp       ← Optional C++ convenience wrapper
...
Create:
  biona/engine/tests/unit/test_signal_schema.cpp
  biona/engine/tests/unit/test_spsc_queue.cpp
  biona/engine/tests/integration/test_pipeline_mock.cpp
  biona/engine/tests/eval/eval_harness.cpp
  biona/engine/tests/CMakeLists.txt
...
# SYSTEM B — Python Training Pipeline
## Track B: Run these prompts in order in `biona/training/`
...
Makefile targets:
  ...
  make shards      → python -m biona_training.data.shards
  make train       → python -m biona_training.training.train
...
Project structure already exists at biona/training/

Create: biona/training/biona_training/data/librispeech.py
...
Also create: biona/training/scripts/download_librispeech.sh
...
CRITICAL: These parameters MUST match the C++ Frontend spec in System A exactly.
Any mismatch causes a silent train/inference accuracy bug.

Create: biona/training/biona_training/data/features.py
...
Also create: biona/training/tests/test_features.py
  Tests:
...
Create: biona/training/biona_training/data/shards.py
...
Also implement: python -m biona_training.data.shards as CLI:
  python -m biona_training.data.shards \
...
CRITICAL CONTRACT — must match biona/engine/core/include/biona/core/onnx_contract.hpp:
...
Create: biona/training/biona_training/model/emformer.py
...
Also create: biona/training/tests/test_model.py
  Tests:
...
Create: biona/training/biona_training/training/train.py
...
8. CLI entry point:
   python -m biona_training.training.train --config train_config.json
   Or: python -m biona_training.training.train (uses defaults)

9. Resume from checkpoint:
   python -m biona_training.training.train --resume checkpoint_00050000.pt
```
...
the model bundle consumed by System A's C++ runtime.

THIS IS THE MOST CRITICAL FILE IN SYSTEM B — it must match the ONNX contract
defined in biona/engine/core/include/biona/core/onnx_contract.hpp exactly.

Create: biona/training/biona_training/model/export.py
...
def encrypt_bundle(onnx_path: Path, output_path: Path,
  ...
  Bundle format matches the EncryptedModelBundle struct in System A.
  Layout: magic(4) + version(4) + iv(12) + tag(16) + hmac(32) + ciphertext
  """

Also create: biona/training/scripts/export_model.py
CLI that:
  1. Loads checkpoint
...
Create: biona/training/biona_training/labeling/semantic_labeler.py
...
Verify that the following constants are IDENTICAL between System A and System B.
Read the actual files and compare — do not assume they match.

System A constants (in biona/engine/core/include/biona/core/onnx_contract.hpp):
...
System B constants (in biona/training/biona_training/data/features.py and
                        biona/training/biona_training/model/emformer.py):
...
and what value it should be changed to. The source of truth is System A's
onnx_contract.hpp — System B must match it, not the other way around.
```

---

*End of Biona Claude CLI Implementation Prompts v1.0*
*Run prompts A-1 through A-12 in Track A, B-1 through B-8 in Track B.*
